...
module DingTalk
  class Request
    ...
    def initialize(base, skip_verify_ssl)
      @base = base
      @httprb = HTTP.timeout(**DingTalk.http_timeout_options)
    ...
    def request(path, header = {}, &_block)
      ...
      parse_response(response, as || :json) do |parse_as, data|
        ...
        result = DingTalk::Result.new(data)
        if defined?(Rails.logger) && Rails.logger
          Rails.logger.debug "[DingTalk] request path(#{url_base}#{path}); request params: #{dup_header.inspect}; response: #{result.inspect}"
        end
        if ENV['DEBUG'] == 'YES'
          puts "[DingTalk] request path(#{url_base}#{path}); request params: #{dup_header.inspect}; response: #{result.inspect}"
        end
